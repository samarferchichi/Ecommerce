...
class CommandesController extends Controller
{
    public function facture()
    {
        ...
        foreach($produits as $produit)
        {
            ...
            $totalHT += $prixHT;
            $totalTTC += $prixTTC + ($produit->getTva()->getValeur()*$produit->getPrix() * $panier[$produit->getId()]);

            if (!isset($commande['tva']['%'.$produit->getTva()->getValeur()]))
                $commande['tva']['%'.$produit->getTva()->getValeur()] = round($prixHT * $produit->getTva()->getValeur(),2);
            else
                $commande['tva']['%'.$produit->getTva()->getValeur()] += round($prixHT * $produit->getTva()->getValeur(),2);
